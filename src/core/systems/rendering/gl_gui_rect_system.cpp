...
{
    for (auto&& [e, guiMesh, glMesh, guiRect] :
         engineState.guiRegistry.view<GuiMesh, const GlMesh, const GuiRect>().each())
    {
        ...
        vertexData[0] = GuiVertex { color, guiRect.topLeft(), { 0, 1 } };
        vertexData[1] = GuiVertex { color, guiRect.bottomLeft(), { 0, 0 } };
        vertexData[2] = GuiVertex { color, guiRect.topRight(), { 1, 1 } };
        vertexData[3] = GuiVertex { color, guiRect.bottomRight(), { 1, 0 } };
